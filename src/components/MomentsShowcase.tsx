...
// "Big moments" — an interactive showcase of the app's celebration animations on
// the marketing page. Each card replays its real burst on tap; the section auto-
// plays one celebration the first time it scrolls into view.
  ...
  return (
    <section id="moments" className="mx-auto max-w-[1180px] px-5 py-[72px]">
      <Reveal className="mx-auto mb-10 max-w-[640px] text-center">
        ...
        <p className="text-[17px] text-ink2">Tap any card to see the moment your table will actually get.</p>
      </Reveal>
      <motion.div
        viewport={{ once: true, amount: 0.4 }}
        onViewportEnter={() => { if (!fired.current) { fired.current = true; celebrate('win', { x: 0.5, y: 0.32 }) } }}
        className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"
      >
        {MOMENTS.map((m) => (
          <Reveal key={m.kind} className="tilt glass edge relative overflow-hidden rounded-[18px]">
            <button type="button" onClick={(e) => celebrate(m.kind, originOf(e.currentTarget))} className="block w-full cursor-pointer text-left">
              ...
              <div className="px-5 pb-5 pt-1">
                ...
                <span className="mt-3 inline-flex items-center gap-1 rounded-full border border-line bg-white/5 px-2.5 py-1 text-[11px] font-bold text-gold">Tap to celebrate ↺</span>
              </div>
            </button>
          </Reveal>
        ))}
